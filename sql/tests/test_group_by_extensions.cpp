...
TEST_CASE("GROUP BY GROUPING SETS - bare single item is canonicalized", "[group-by][grouping-sets]") {
...
TEST_CASE("GROUP BY - full clause tail still parses after extensions", "[group-by][mixed]") {
    REQUIRE(transpile("SELECT a, SUM(b) FROM t GROUP BY ROLLUP(a) HAVING SUM(b) > 1 ORDER BY a LIMIT 5")
...
TEST_CASE("GROUP BY extensions do not relax trailing-input checking", "[group-by][strict]") {
    ...
    SQLParser parser2(arena2, "SELECT a FROM t GROUP BY GROUPING SETS ((a)", SQLDialect::PostgreSQL);
