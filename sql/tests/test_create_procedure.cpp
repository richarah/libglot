...
TEST_CASE("CREATE PROCEDURE parsing and generation", "[procedural][create_procedure]") {
    ...
    SECTION("CREATE PROCEDURE with parameters") {
        std::string sql = "CREATE PROCEDURE add_user(name VARCHAR(100), age INT) AS BEGIN INSERT "
                          "INTO users VALUES (name, age); END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("CREATE FUNCTION with RETURNS clause") {
        std::string sql =
            "CREATE FUNCTION get_total(x INT, y INT) RETURNS INT AS BEGIN RETURN x + y; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("CREATE FUNCTION with LANGUAGE clause (PostgreSQL)") {
        std::string sql =
            "CREATE FUNCTION test_func() RETURNS INT LANGUAGE plpgsql AS BEGIN RETURN 42; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("CREATE PROCEDURE with IN/OUT parameters") {
        std::string sql =
            "CREATE PROCEDURE swap(IN a INT, OUT b INT, INOUT c INT) AS BEGIN SELECT a; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("CREATE PROCEDURE with multiple statements in body") {
        std::string sql =
            "CREATE PROCEDURE multi_stmt() AS BEGIN SELECT 1; SELECT 2; SELECT 3; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
...
TEST_CASE("CREATE PROCEDURE dialect transpilation", "[procedural][create_procedure][dialects]") {
    SECTION("PostgreSQL PL/pgSQL procedure") {
        std::string sql = "CREATE OR REPLACE FUNCTION calculate_total(amount INT) RETURNS INT "
                          "LANGUAGE plpgsql AS BEGIN RETURN amount * 2; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("MySQL stored procedure") {
        std::string sql =
            "CREATE PROCEDURE update_count() AS BEGIN UPDATE counters SET value = value + 1; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
        ...
        // SQLGenerator quotes identifiers
    ...
    SECTION("Oracle PL/SQL procedure") {
        std::string sql =
            "CREATE OR REPLACE PROCEDURE test_proc(p_id IN INT) AS BEGIN SELECT p_id; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
...
TEST_CASE("CREATE FUNCTION with procedural logic", "[procedural][create_function]") {
    SECTION("Function with IF statement") {
        std::string sql = "CREATE FUNCTION check_value(x INT) RETURNS INT AS BEGIN IF x > 10 THEN "
    ...
    SECTION("Function with WHILE loop") {
        std::string sql = "CREATE FUNCTION count_up(n INT) RETURNS INT AS BEGIN DECLARE i INT; "
                          "WHILE i < n DO SELECT i; END WHILE; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
    ...
    SECTION("Function with FOR loop") {
        std::string sql = "CREATE FUNCTION iterate(n INT) RETURNS INT AS BEGIN FOR i IN 1..n LOOP "
                          "SELECT i; END LOOP; END";
        auto result = [&]() {
            libglot::Arena arena;
            SQLParser parser(arena, sql);
            auto ast = parser.parse_top_level();
