...
TEST_CASE("Regression - quoted identifiers re-parse without doubling", "[regression][quoting]") {
    ...
    REQUIRE(transpile("SELECT `id` FROM `users`", SQLDialect::MySQL)
            == "SELECT `id` FROM `users`");
...
TEST_CASE("Regression - doubled quote inside quoted identifier is unescaped", "[regression][quoting]") {
...
TEST_CASE("Regression - CURRENT_* are keyword expressions, not strings", "[regression][literal]") {
    ...
    REQUIRE(transpile("CREATE TABLE t (created TIMESTAMP DEFAULT CURRENT_TIMESTAMP)", SQLDialect::PostgreSQL)
...
TEST_CASE("Regression - EXTRACT keeps field keyword and FROM form", "[regression][extract]") {
    ...
    REQUIRE(transpile("SELECT CAST(EXTRACT(YEAR FROM d) AS VARCHAR(10)) FROM t", SQLDialect::PostgreSQL)
...
TEST_CASE("Regression - SQL Server OFFSET requires OFFSET..FETCH after ORDER BY", "[regression][limit]") {
    // No TOP + OFFSET mix
    REQUIRE(transpile("SELECT * FROM users ORDER BY id LIMIT 10 OFFSET 20", SQLDialect::SQLServer)
...
TEST_CASE("Regression - OFFSET..FETCH and FETCH FIRST forms parse everywhere", "[regression][limit]") {
    ...
    REQUIRE(transpile("SELECT * FROM users OFFSET 5 ROWS FETCH NEXT 10 ROWS ONLY", SQLDialect::MySQL)
...
TEST_CASE("Regression - ILIKE routed through LOWER() polyfill where unsupported", "[regression][ilike]") {
...
TEST_CASE("Regression - null-safe equality and ASOF joins", "[regression][dialect-ops]") {
    REQUIRE(transpile("SELECT a <=> b FROM t", SQLDialect::MySQL)
            == "SELECT `a` <=> `b` FROM `t`");
