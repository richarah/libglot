...
TEST_CASE("Temporary tables - PostgreSQL", "[temp][postgresql]") {
    ...
    SECTION("CREATE TEMPORARY TABLE") {
        std::string sql =
            "CREATE TEMPORARY TABLE session_data (user_id INT, session_key VARCHAR(255))";
        std::string result = test_round_trip(sql);
...
TEST_CASE("Temporary tables - MySQL", "[temp][mysql]") {
    ...
    SECTION("CREATE TEMPORARY TABLE with ENGINE") {
        std::string sql =
            "CREATE TEMPORARY TABLE cache (key_name VARCHAR(100), value TEXT) ENGINE=MEMORY";
        libglot::Arena arena;
        SQLParser parser(arena, sql);
        auto ast = parser.parse_top_level();
        REQUIRE(ast != nullptr);
    ...
    SECTION("CREATE TEMPORARY TABLE AS SELECT") {
        std::string sql = "CREATE TEMPORARY TABLE recent_orders AS SELECT * FROM orders WHERE "
...
TEST_CASE("Temporary tables - Oracle", "[temp][oracle]") {
    SECTION("CREATE GLOBAL TEMPORARY TABLE") {
        std::string sql =
            "CREATE GLOBAL TEMPORARY TABLE temp_session (user_id NUMBER, session_data CLOB)";
        libglot::Arena arena;
        SQLParser parser(arena, sql);
        auto ast = parser.parse_top_level();
        REQUIRE(ast != nullptr);
    ...
    SECTION("CREATE GLOBAL TEMPORARY TABLE ON COMMIT DELETE ROWS") {
        std::string sql =
            "CREATE GLOBAL TEMPORARY TABLE temp_calc (result NUMBER) ON COMMIT DELETE ROWS";
        libglot::Arena arena;
        SQLParser parser(arena, sql);
        auto ast = parser.parse_top_level();
        REQUIRE(ast != nullptr);
    ...
    SECTION("CREATE GLOBAL TEMPORARY TABLE ON COMMIT PRESERVE ROWS") {
        std::string sql = "CREATE GLOBAL TEMPORARY TABLE temp_staging (data VARCHAR2(4000)) ON "
...
TEST_CASE("Temporary tables - BigQuery", "[temp][bigquery]") {
    SECTION("CREATE TEMP TABLE") {
        std::string sql =
            "CREATE TEMP TABLE temp_results AS SELECT id, name FROM users WHERE active = TRUE";
    ...
    SECTION("CREATE TEMPORARY TABLE") {
        std::string sql =
            "CREATE TEMPORARY TABLE session_data (user_id INT64, timestamp TIMESTAMP)";
        std::string result = test_round_trip(sql, SQLDialect::BigQuery);
        REQUIRE(result.find("TEMPORARY") != std::string::npos);
    }
}
...
TEST_CASE("Temporary tables - Common operations", "[temp][operations]") {
    SECTION("Insert into temporary table") {
        std::string sql =
            "INSERT INTO temp_users SELECT * FROM users WHERE created_at > '2024-01-01'";
        libglot::Arena arena;
        SQLParser parser(arena, sql);
        auto ast = parser.parse_top_level();
        REQUIRE(ast != nullptr);
