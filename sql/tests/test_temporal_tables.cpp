...
TEST_CASE("Temporal table - AS OF", "[temporal]") {
    REQUIRE(transpile("SELECT * FROM t FOR SYSTEM_TIME AS OF '2020-01-01'", SQLDialect::SQLServer)
            == "SELECT * FROM [t] FOR SYSTEM_TIME AS OF '2020-01-01'");
    REQUIRE(transpile("SELECT * FROM t FOR SYSTEM_TIME AS OF '2020-01-01'", SQLDialect::AzureSynapse)
            == "SELECT * FROM \"t\" FOR SYSTEM_TIME AS OF '2020-01-01'");
...
TEST_CASE("Temporal table - BETWEEN ... AND ...", "[temporal]") {
    REQUIRE(transpile("SELECT * FROM t FOR SYSTEM_TIME BETWEEN 'a' AND 'b'", SQLDialect::SQLServer)
            == "SELECT * FROM [t] FOR SYSTEM_TIME BETWEEN 'a' AND 'b'");
}

TEST_CASE("Temporal table - CONTAINED IN (...)", "[temporal]") {
    REQUIRE(transpile("SELECT * FROM t FOR SYSTEM_TIME CONTAINED IN ('a', 'b')", SQLDialect::SQLServer)
...
TEST_CASE("Temporal table - clause comes before the alias", "[temporal]") {
    REQUIRE(transpile("SELECT * FROM t FOR SYSTEM_TIME AS OF '2020-01-01' AS t1", SQLDialect::SQLServer)
            == "SELECT * FROM [t] FOR SYSTEM_TIME AS OF '2020-01-01' AS [t1]");
}

TEST_CASE("Temporal table - AST shape", "[temporal]") {
    libglot::Arena arena;
    SQLParser parser(arena, "SELECT * FROM t FOR SYSTEM_TIME AS OF '2020-01-01'", SQLDialect::SQLServer);
...
TEST_CASE("Temporal table - unsupported dialects throw a clean std::logic_error", "[temporal][error]") {
    for (auto d : {SQLDialect::PostgreSQL, SQLDialect::MySQL, SQLDialect::Oracle, SQLDialect::ANSI}) {
...
TEST_CASE("Temporal table - bad syntax raises a clean ParseError", "[temporal][error]") {
    libglot::Arena arena;
    SQLParser parser(arena, "SELECT * FROM t FOR SYSTEM_TIME SNAPSHOT '2020-01-01'", SQLDialect::SQLServer);
