...
TEST_CASE("MATCH AGAINST - IN BOOLEAN MODE", "[fulltext]") {
    REQUIRE(transpile(
                "SELECT * FROM articles WHERE MATCH (title) AGAINST ('+database -mysql' IN BOOLEAN MODE)",
                SQLDialect::MySQL)
            == "SELECT * FROM `articles` WHERE MATCH (`title`) AGAINST ('+database -mysql' IN BOOLEAN MODE)");
}
...
TEST_CASE("MATCH AGAINST - AST shape", "[fulltext]") {
    ...
    SQLParser parser(arena,
        "SELECT * FROM t WHERE MATCH (a, b) AGAINST ('x' IN BOOLEAN MODE)", SQLDialect::MySQL);
...
TEST_CASE("MATCH AGAINST - fixed point (MySQL/MariaDB)", "[fulltext][roundtrip]") {
    const std::string queries[] = {
        ...
        "SELECT * FROM t WHERE MATCH (a) AGAINST ('x' IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION)",
...
TEST_CASE("MATCH AGAINST - unsupported dialects throw a clean std::logic_error", "[fulltext][error]") {
    for (auto d : {SQLDialect::PostgreSQL, SQLDialect::SQLServer, SQLDialect::ANSI, SQLDialect::Oracle}) {
        REQUIRE_THROWS_AS(
            transpile("SELECT * FROM t WHERE MATCH (a) AGAINST ('x')", d), std::logic_error);
    }
}
...
TEST_CASE("MATCH AGAINST - bad modifier is a clean ParseError", "[fulltext][error]") {
    libglot::Arena arena;
    SQLParser parser(arena, "SELECT * FROM t WHERE MATCH (a) AGAINST ('x' IN WEIRD MODE)", SQLDialect::MySQL);
    REQUIRE_THROWS_AS(parser.parse_top_level(), libglot::ParseError);
}
