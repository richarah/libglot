...
TEST_CASE("Named window - exact string", "[named-window]") {
    REQUIRE(gen("SELECT a, ROW_NUMBER() OVER w FROM t WINDOW w AS (PARTITION BY a ORDER BY b)",
                SQLDialect::ANSI)
            == "SELECT \"a\", ROW_NUMBER() OVER \"w\" FROM \"t\" "
               "WINDOW \"w\" AS (PARTITION BY \"a\" ORDER BY \"b\")");
}

TEST_CASE("Named window - multiple named windows", "[named-window]") {
    REQUIRE(gen("SELECT a FROM t WINDOW w1 AS (PARTITION BY a), w2 AS (ORDER BY b)", SQLDialect::ANSI)
...
TEST_CASE("Named window - generated SQL is a fixed point in every dialect", "[named-window][fixpoint]") {
    const std::string q = "SELECT a, ROW_NUMBER() OVER w FROM t WINDOW w AS (PARTITION BY a ORDER BY b)";
