...
public:
    ...
    explicit SQLOptimizer(libglot::Arena& arena) : arena_(arena), options_() {}
    ...
    SQLNode* optimize(SQLNode* node) { return opt(node); }
...
private:
    ...
    SQLNode* opt(SQLNode* node) {
        ...
        case SQLNodeKind::BINARY_OP: {
            ...
            if (options_.fold_constants) {
                if (SQLNode* folded = fold_binary(op))
                    return folded;
            }
            if (options_.simplify_booleans) {
                if (SQLNode* simplified = simplify_boolean_binary(op))
                    return simplified;
            }
            return op;
        }
        ...
        case SQLNodeKind::UNARY_OP: {
            ...
            if (options_.simplify_booleans && op->op == TK::NOT) {
                if (is_bool_literal(op->operand, "TRUE"))
                    return make_bool_literal(false);
                if (is_bool_literal(op->operand, "FALSE"))
        ...
        case SQLNodeKind::SELECT_STMT: {
            ...
            if (stmt->qualify)
                stmt->qualify->condition = opt(stmt->qualify->condition);
            ...
            if (stmt->start_with)
                stmt->start_with->condition = opt(stmt->start_with->condition);
            if (stmt->connect_by)
                stmt->connect_by->condition = opt(stmt->connect_by->condition);
            return stmt;
        }
        ...
        case SQLNodeKind::CREATE_TABLE_STMT: {
            ...
            for (auto* constraint : stmt->constraints) {
                if (constraint)
                    constraint->check_expr = opt(constraint->check_expr);
            }
            return stmt;
        }
    ...
    SQLNode* simplify_boolean_binary(BinaryOp* op) {
        if (op->op == TK::AND) {
            if (is_bool_literal(op->left, "TRUE"))
                return op->right; // TRUE AND x -> x
            if (is_bool_literal(op->right, "TRUE"))
                return op->left; // x AND TRUE -> x
            if (is_bool_literal(op->left, "FALSE"))
                return op->left; // FALSE AND x -> FALSE
            if (is_bool_literal(op->right, "FALSE"))
                return op->right; // x AND FALSE -> FALSE
        } else if (op->op == TK::OR) {
            if (is_bool_literal(op->left, "FALSE"))
                return op->right; // FALSE OR x -> x
            if (is_bool_literal(op->right, "FALSE"))
                return op->left; // x OR FALSE -> x
            if (is_bool_literal(op->left, "TRUE"))
                return op->left; // TRUE OR x -> TRUE
            if (is_bool_literal(op->right, "TRUE"))
                return op->right; // x OR TRUE -> TRUE
        }
        return nullptr;
    }
    ...
    static bool is_int_literal(const SQLNode* node, long long& value) noexcept {
        if (!node || node->type != SQLNodeKind::LITERAL)
            return false;
    ...
    SQLNode* make_bool_literal(bool value) {
        ...
        return arena_.create<Literal>(value ? std::string_view{"TRUE"} : std::string_view{"FALSE"});
    }
};
