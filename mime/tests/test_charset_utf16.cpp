...
TEST_CASE("UTF-16: default endianness is big-endian per RFC 2781 when no BOM is given", "[charset][utf16]") {
...
TEST_CASE("UTF-16: surrogate pair decodes an astral codepoint (emoji)", "[charset][utf16][surrogates]") {
...
TEST_CASE("UTF-16: surrogate pair round-trips surrounded by ASCII text", "[charset][utf16][surrogates]") {
    std::string input = utf16be({0x0048, 0xD83D, 0xDE00, 0x0021});  // "H" emoji "!"
    std::string utf8 = CharsetConverter::utf16_to_utf8(input, Endianness::Big);
    REQUIRE(utf8 == "H\xF0\x9F\x98\x80!");
}

TEST_CASE("UTF-16: unpaired high surrogate becomes U+FFFD, never crashes", "[charset][utf16][surrogates][security]") {
    ...
    REQUIRE(utf8 == "\xEF\xBF\xBD" "A");  // U+FFFD then 'A'
    REQUIRE(CharsetConverter::is_valid_utf8(utf8));
}

TEST_CASE("UTF-16: unpaired high surrogate at end of input becomes U+FFFD", "[charset][utf16][surrogates][security]") {
...
TEST_CASE("UTF-16: unpaired low surrogate becomes U+FFFD, never crashes", "[charset][utf16][surrogates][security]") {
    // Low surrogate DC00 with no preceding high surrogate
    std::string input = utf16be({0xDC00, 0x0041});
    std::string utf8;
    REQUIRE_NOTHROW(utf8 = CharsetConverter::utf16_to_utf8(input, Endianness::Big));
    REQUIRE(utf8 == "\xEF\xBF\xBD" "A");
    REQUIRE(CharsetConverter::is_valid_utf8(utf8));
}

TEST_CASE("UTF-16: two consecutive high surrogates each become U+FFFD", "[charset][utf16][surrogates][security]") {
...
TEST_CASE("UTF-16: odd trailing byte becomes U+FFFD, never crashes or reads out of bounds", "[charset][utf16][security]") {
...
TEST_CASE("UTF-16: single stray odd byte (no complete code unit at all)", "[charset][utf16][security]") {
...
TEST_CASE("UTF-16: detect_charset recognizes UTF-16/UTF-16BE/UTF-16LE case-insensitively", "[charset][utf16]") {
...
TEST_CASE("Pipeline: text/plain part with charset=UTF-16BE decodes to UTF-8", "[mime][pipeline][utf16]") {
    libglot::Arena arena;
...
TEST_CASE("Pipeline: text/plain part with charset=UTF-16LE decodes to UTF-8", "[mime][pipeline][utf16]") {
    libglot::Arena arena;
...
TEST_CASE("Pipeline: text/plain part with bare charset=UTF-16 (BOM) decodes to UTF-8", "[mime][pipeline][utf16]") {
    libglot::Arena arena;
