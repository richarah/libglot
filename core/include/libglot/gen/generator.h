...
public:
    ...
    explicit GeneratorBase(DialectId dialect, const Options& opts = Options{})
        : dialect_(dialect), features_(DialectTraitsType::get_features(dialect)), options_(opts),
          output_(), indent_level_(0) {}
    ...
    /// Get current output
    [[nodiscard]] std::string current_output() const { return output_.str(); }
...
protected:
    ...
    void write_list(const std::vector<T>& items, GenerateFunc generator,
    ...
    template<typename T, typename GenerateFunc>
    void write_list_multiline(const std::vector<T>& items, GenerateFunc generator,
    ...
    template<typename T, typename GenerateFunc>
    void write_paren_list(const std::vector<T>& items, GenerateFunc generator,
                          const char* separator = ", ") {
        write('(');
    ...
    void write_identifier(std::string_view ident) { write(ident); }
    ...
    [[nodiscard]] DialectId dialect() const noexcept { return dialect_; }

    [[nodiscard]] const Features& features() const noexcept { return features_; }
